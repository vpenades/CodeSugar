...
using READABLEBLOCK = System.IO.Stream;
using WRITEABLEBLOCK = System.IO.Stream;
...
#endif
{
    static partial class CodeSugarForSerialization
    {
        ...
        public delegate WRITEABLEBLOCK ItemWriteStreamDelegate<T>(WRITEABLEBLOCK source, T item);
        public delegate READABLEBLOCK ItemReadStreamDelegate<T>(READABLEBLOCK source, ref T item);

        public static WRITEABLEBLOCK WriteList<T>(this WRITEABLEBLOCK target, IReadOnlyList<T> source, ItemWriteStreamDelegate<T> lambda)
        {
        ...
        public static READABLEBLOCK ReadList<T>(this READABLEBLOCK source, ref List<T> result, ItemReadStreamDelegate<T> lambda)
        {
        ...
        public static READABLEBLOCK ReadList<T>(this READABLEBLOCK source, IList<T> result, ItemReadStreamDelegate<T> lambda)
        {
            source = source.ReadPackedU64(out var count);
            if (count == 0) return source;            
        ...
        public static READABLEBLOCK ReadList<T>(this READABLEBLOCK source, out T[] result, ItemReadStreamDelegate<T> lambda)
        {
            source = source.ReadPackedU64(out var count);
        ...
        public static WRITEABLEBLOCK WriteDictionary<T>(this WRITEABLEBLOCK target, IReadOnlyDictionary<string, T> source, ItemWriteStreamDelegate<T> lambda)
        {
            if (source == null) return target.WritePackedU64(0);
        ...
        public static READABLEBLOCK ReadDictionary<T>(this READABLEBLOCK source, ref Dictionary<string, T> result, ItemReadStreamDelegate<T> lambda)
        {
            result ??= new Dictionary<string,T>();
            return ReadDictionary(source, result, lambda);
        }

        public static READABLEBLOCK ReadDictionary<T>(this READABLEBLOCK source, IDictionary<string, T> result, ItemReadStreamDelegate<T> lambda)
        {
            source = source.ReadPackedU64(out var count);
            if (count == 0) return source;            
