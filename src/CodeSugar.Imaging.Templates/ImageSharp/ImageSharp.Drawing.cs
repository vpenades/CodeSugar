    ...
    partial class CodeSugarImagingExtensions
    {
            ...
            where TSrcPixel : unmanaged, IPixel<TSrcPixel>
        {
            ...
            */
            ...
            var xformer = new _PixelSamplerTransform(foregroundTransform);
            var sampler = new _ImageSharpBilinearSampler<TSrcPixel>(foreground);
            ...
            void _processRowWithAlpha(Span<Vector4> dstSpan, Point value)
            {
                for (int i = 0; i < dstSpan.Length; i++)
                {
                    ...
                    dstSpan[i] = _ImageSharpComposeScaledVectorNormal(dstSpan[i], forePixel);
                }
            }
            ...
            if (_HasAlphaChannel<TSrcPixel>() || opacity != 1)
            {
